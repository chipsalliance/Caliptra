...
3. Changing the security state at a random time after cptra\_rst\_b deassertion: transition from debug locked to debug unlocked has no effect until next reset.
4. Setting scan mode at a random time after cptra\_pwrgood: flushes out all internal Caliptra secrets, assets, key vault, and crypto intermediate state.
5. Scan mode assertion does not automatically open Caliptra JTAG.
...
* The SoC (via the SOC Manager mutable code, outside the scope of Caliptra) provisions a per-device symmetric seed, `PQ.DevID.Seed` (at least 384 bits, with the same protection expectations as other per-device secrets), to Caliptra runtime firmware using the [`SET_PQ_SEED`](#set_pq_seed) mailbox command.
* Runtime firmware mixes `PQ.DevID.Seed` through an HMAC-based KDF to derive `PQ.DevID.CDI`, which is cached in persistent data for the remainder of the boot session.
* `PQ.DevID.CDI` is used to deterministically derive an ML-DSA-87 keypair (same seed always produces the same keypair, which enables KAT-style testing and re-derivation across resets without re-provisioning).
...
* `PQ.DevID` provides a post-quantum-safe (ML-DSA-87 / FIPS 204) device identity and signing capability, usable for SPDM 1.4 PQC algorithm negotiation and attestation, and for DPE-mediated leaf identities, alongside — not instead of — the existing ECDSA DICE identity.
* `PQ.DevID.Seed` delivery is restricted to PL0 callers only (`SET_PQ_SEED` calls `ensure_pl0()`), and the derived `PQ.DevID.CDI` and cached key material live only in Caliptra's persistent data (DCCM) for the runtime session; the ML-DSA-87 private key itself is never exposed across the mailbox.
* **`SET_PQ_SEED` must only be issued while Caliptra is in a `DebugLocked` security state, and never while `DebugUnlocked`.** This is a requirement on the SoC provisioning flow, not something runtime firmware enforces on its own: `SetPqSeedCmd::execute` checks the caller's PL0 privilege level but never calls `debug_locked()`, so nothing in the command itself refuses to accept the seed over JTAG-open silicon. This matters because `PQ.DevID.Seed`/`PQ.DevID.CDI` get none of the hardware protection the classical DICE secrets rely on: transitioning to `DebugUnlocked` reverts UDS to a well-known debug value, switches to the debug obfuscation key, and clears the hardware Key Vault (see [Caliptra security states](#caliptra-security-states)), but `PQ.DevID.CDI` is ordinary software state in DCCM with no analogous debug-triggered scrub. Since security state is latched at reset and cannot change mid-session (see [JTAG/TAP debug](#jtagtap-debug)), a device provisioned — or merely booted to runtime firmware — with debug open exposes both the in-flight seed on the mailbox and the resident `PQ.DevID.CDI`/keypair to the same JTAG register/memory access that `DebugUnlocked` grants, for the remainder of that boot session.
* Because keypair derivation is deterministic from `PQ.DevID.Seed`, the same identity is reproducible across Caliptra resets and runtime firmware updates without requiring the SoC to re-provision a new `PQ.DevID` certificate. The same determinism raises the stakes of the debug-mode requirement above: unlike a randomly re-rolled key, a `PQ.DevID.Seed` (or CDI) captured even once lets an attacker re-derive the full ML-DSA-87 keypair at will, permanently and silently impersonating that device's `PQ.DevID` identity — there is no forward secrecy, and no in-band way for a verifier to detect that the seed was exposed.
...
Because ML-DSA-87 in Caliptra 1.3 is implemented entirely in software on the VeeR core, several of the new commands are measurably more expensive than their ECDSA/LMS counterparts, and this cost cannot be fully eliminated without hardware acceleration. `CERTIFY_KEY_EXTENDED_MLDSA87` in particular is the most expensive command in the mailbox surface — an order of magnitude more than `CERTIFY_KEY_EXTENDED` — with a tail cost of roughly 231,529,190 CPU cycles measured by the runtime's cycle-count regression tests. At the reference 400 MHz core clock used by these tests (also the basis for the extended PQC mailbox-command watchdog budget below), that is **just over 0.5 seconds**; actual latency on a given integration will vary with the SoC's core clock frequency and system load, and should not be assumed to be lower.

A substantial number of optimizations targeting cycle count, code size, and stack usage have already been implemented and landed on the `caliptra-1.x` branch — including removing on-stack copies of intermediate values, merging redundant computation passes (for example, folding `mu` generation into signing), applying `#[inline(never)]` to control code placement pressure, caching the ML-DSA-87 `A`-matrix polynomials computed during signing so the rejection-sampling loop avoids redundant Keccak passes on each retry (at a cost of additional stack, tunable via a configuration constant), extending the mailbox command watchdog specifically for PQC commands (`extend_wdt_for_pqc`, used by `CERTIFY_KEY_EXTENDED_MLDSA87`, `GET_PQ_CSR`, `GET_PQ_INFO`, `INVOKE_DPE_MLDSA87`, and `SIGN_WITH_EXPORTED_MLDSA` to raise their watchdog budget past the default 20-million-cycle limit), and adding dedicated cycle-count and stack-usage regression tests to prevent silent regressions. Despite this work, the fundamental cost of software lattice cryptography remains: the underlying tension is a three-way tradeoff between execution performance, code size, and stack usage, and further gains in one dimension generally come at the expense of another. Integrators evaluating this feature should budget for this latency (particularly for `CERTIFY_KEY_EXTENDED_MLDSA87`) in any timing-sensitive attestation flow, and should not assume PQC command latency is comparable to the existing ECDSA/LMS commands.
