...
* Because keypair derivation is deterministic from `PQ.DevID.Seed`, the same identity is reproducible across Caliptra resets and runtime firmware updates without requiring the SoC to re-provision a new `PQ.DevID` certificate. The same determinism raises the stakes of the debug-mode requirement above: unlike a randomly re-rolled key, a `PQ.DevID.Seed` (or CDI) captured even once lets an attacker re-derive the full ML-DSA-87 keypair at will, permanently and silently impersonating that device's `PQ.DevID` identity — there is no forward secrecy, and no in-band way for a verifier to detect that the seed was exposed.
  This is the same one-shot-capture risk that [UDS](#uds) and field entropy (see [Commentary: threat analysis](#commentary-threat-analysis)) accept for the classical ECDSA IDevID/LDevID chain — both are also static secrets whose exposure lets an attacker deterministically re-derive a device identity forever, with no forward secrecy either. What differs is the hardware backstop and the recovery path. UDS and field entropy are stored obfuscated in fuses and are only ever deobfuscated inside the Deobfuscation Engine using an obfuscation key known solely to Caliptra hardware; the plaintext secret and everything derived from it are confined to the hardware Key Vault, never readable by software, and are scrubbed automatically on `DebugUnlocked` transitions (see [Caliptra security states](#caliptra-security-states)). `PQ.DevID.Seed` has no obfuscation-key or fuse-bound equivalent: it is handed to runtime firmware in the clear over the mailbox by the SoC, and `PQ.DevID.CDI` and the derived ML-DSA-87 keypair live as ordinary, software-readable bytes in DCCM rather than in the Key Vault, so there is no hardware wall between the secret and any code capable of reading Caliptra's persistent data. Classical UDS exposure also has a partial recovery path that `PQ.DevID` lacks: because LDevID is derived from both UDS and field entropy, an owner who suspects UDS was captured during manufacturing can burn a fresh field entropy fuse slot during onboarding to mint a new LDevID that a UDS-only attacker cannot reproduce; `PQ.DevID`, seeded once from a single flat `PQ.DevID.Seed` with no analogous second factor, has no comparable in-field rotation mechanism, so a captured seed is a permanent, unrecoverable compromise of that identity (see [Commentary: threat analysis](#commentary-threat-analysis)).
...
* **`PQ.DevID` deliberately does not bind to Caliptra's own measurements.** Neither the FMC hash nor the RT hash is mixed into `PQ.DevID.Seed`/`PQ.DevID.CDI`, unlike the ECDSA chain where Alias<sub>FMC</sub> and Alias<sub>RT</sub> are cryptographically bound to firmware measurements. This means **Caliptra's own firmware integrity is not attested by the PQC identity** — a `PQ.DevID`-signed certificate or signature says nothing about which FMC/RT is running. This tradeoff was made deliberately: binding to firmware measurements would require re-provisioning the `PQ.DevID` certificate (a costly manufacturing-flow operation) on every FMC/RT firmware update.
  This does **not** mean PQC-signed attestation of Caliptra's firmware is entirely unavailable. `INVOKE_DPE_MLDSA87` and `CERTIFY_KEY_EXTENDED_MLDSA87` operate on the same DPE context/TCI tree as their ECDSA counterparts (see [New mailbox commands](#new-mailbox-commands) below), whose root TCI is seeded from `RT_FW_CURRENT_PCR`/`RT_FW_JOURNEY_PCR` — PCRs extended with the **RT firmware hash** alone.
  Integrators who need PQC-signed attestation of the **RT** measurement should request it through DPE-mediated ML-DSA-87 leaf certificates rather than treating the bare `PQ.DevID` certificate as an attestation.
