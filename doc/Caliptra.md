...
To ensure quantum-resistant RTM, each certificate includes a hybrid signature based on the IETF composite signature specification.
...
Caliptra shall support field-programmable entropy, which factors into the device's LDevID identity. The [LDevID certificate](#ldevid-certificate) is a hybrid certificate endorsed by IDevID and in turn endorses the FMC alias key.
...
| Issuer Name                     | CN           | Caliptra 2.0 IDevID
|                                 | serialNumber | Hex-encoded printable string of SHA256 hash of DER-formatted IDevID public key in uncompressed form
| Validity                        | notBefore    | 20230101000000Z
|                                 | notAfter     | 99991231235959Z
| Subject Name                    | CN           | Caliptra 2.0 LDevID
|                                 | serialNumber | Hex-encoded printable string of SHA256 hash of DER-formatted LDevID public key in uncompressed form
| Subject Public Key1 Info        | Algorithm    | ML-DSA-87
|                                 | Parameters   | Category 5 (k,l) = (8,7)
|                                 | Public Key   | LDevID Public Key1 value
| Subject Public Key2 Info        | Algorithm    | ecdsa-with-SHA384
|                                 | Parameters   | Named Curve = prime384v1
|                                 | Public Key   | LDevID Public Key2 value
| Signature Algorithm1 Identifier | Algorithm    | ML-DSA-87
|                                 | Parameters   | Category 5 (k,l) = (8,7)
| Signature Algorithm2 Identifier | Algorithm    | ecdsa-with-SHA384
|                                 | Parameters   | Named Curve = prime384v1
| Signature Value                 | -            | Digital signature for the certificate
| KeyUsage                        | keyCertSign  | 2
| Basic Constraints               | CA           | True
|                                 | pathLen      | 4
...
| Subject Public Key1 Info        | Algorithm    | ML-DSA-87
|                                 | Parameters   | Category 5 (k,l) = (8,7)
|                                 | Public Key   | RT Alias Public Key1 value
| Subject Public Key2 Info        | Algorithm    | ecdsa-with-SHA384
|                                 | Parameters   | Named Curve = prime384v1
|                                 | Public Key   | RT Alias Public Key2 value
| Signature Algorithm1 Identifier | Algorithm    | ML-DSA-87
|                                 | Parameters   | Category 5 (k,l) = (8,7)
| Signature Algorithm2 Identifier | Algorithm    | ecdsa-with-SHA384
|                                 | Parameters   | Named Curve = prime384v1
...
[^8]: ECDSA and MLDSA is used for firmware verification and SPDM (signing).

[^9]: SHA is used with ECDSA and HMAC, and is also used to generate measurements.

[^10]: The id-MLDSA87-ECDSA-P384-SHA512 is a composite signature that utilizes the PQC ML-DSA-87 and ECC Secp384r1 algorithms, pre-hashed by SHA512.

[^11]: Any SHA/HMAC operation in PQC ML-DSA-87 path must use SHA512/HMAC512.