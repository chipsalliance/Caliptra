...
> **Living working draft.** This file on `main` is the working draft of the Caliptra Trademark Audit Process. To cite a frozen version of this document for compliance purposes, refer to the corresponding `edition/<identifier>` branch and `edition-<identifier>-r<N>` tag listed in the [Trademark Edition Index](doc/trademark/README.md). See the [Caliptra Trademark Versioning Scheme](doc/trademark/CaliptraTrademarkVersioningScheme.md) for details.
...
The objective of the Caliptra Trademark Auditing Process is to provide guidelines to Caliptra instantiation, with focus on the most important security aspects and properties for “real-world” security, based on the security objectives and use cases of Caliptra. It is expected that the audit process will be less detailed than the NIST certification requirements.

This document defines the process to be used by auditors to determine if the vendor’s products meet the Caliptra trademark requirements. Auditors should focus on analyzing vendor architecture and implementation of Caliptra on vendor products, rather than testing of vendor products. Vendors should complete the requirements in this document to request permission to use the Caliptra Trademark. The [Checklist and evaluation methodology document](https://github.com/chipsalliance/Caliptra/blob/main/doc/trademark/CaliptraChecklistAndEvaluationMethodology.md) can be used by auditors to determine if the vendor is complying with the Caliptra Integration Specification. To demonstrate compliance with the Caliptra Integration Requirements, vendors must provide necessary collateral to auditors for each integration requirement, as defined in the checklist. Auditors must review the materials and prepare a Statement of Conformance that attests to integrator compliance with all requirements. The main security functionality of Caliptra as a silicon RoT, when correctly integrated, is to:
...
4. The laboratory and manufacturer communicate and jointly, when aligned, provide an [application](Caliptra%20Conformance%20Program%20-%20Participation%20Form.pdf) for Caliptra Trademark Usage for a TOE to Caliptra Trademark Owner TAC, signed by manufacturer. The manufacturer sends the signed application to the Caliptra Trademark Owner at <caliptra-tm@lists.chipsalliance.org>. Note that this application form is also used to sign up for the Caliptra Vulnerability Alerts Mailing List.
