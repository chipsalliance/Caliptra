...
- A configurable number of **multi-party authorities**, which each represent an additional entity that must assent before user data is available. These authorities grant access to the data by providing an access key.
  ...
  - Each MPAK has a distinct access key, which is never stored persistently within the drive and which is protected in transit using HPKE[[5]]. This enables use-cases where the access key is served to the drive from a remote key management service, without revealing the access key to the drive's host.
