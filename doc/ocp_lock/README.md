...
To safeguard user data stored on the drive, KMB defines a set of "authorities", each of which can unilaterally withhold access to the data.

- The **data authority**, which represents the nominal owner of the data. This authority grants access to the data by providing, e.g., their Opal C_PIN or Key Per I/O DEK.
  - KMB represents this authority in terms of the **Data Authority Key (DAK)**. A given MEK is bound to a single DAK. In Opal the DAK may be protected by the user's C_PIN, while in Key Per I/O it may be the DEK associated with a given key tag.

- A configurable number of **multi-party authorities**, which each represent an additional entity that must assent before user data is available. These authorities grant access to the data by providing an access key.
  - KMB represents these authorities in terms of **Multi-Party Authority Keys (MPAKs)**. A given MEK may be bound to zero or more MPAKs. Binding an MEK to zero MPAKs allows for legacy Opal or Key Per I/O flows.
  - Each MPAK has a distinct access key, which is never stored persistently within the drive and which is protected in transit using HPKE[\[5\]](#ref-5). This enables use-cases where the access key is served to the drive from a remote key management service, without revealing the access key to the drive's host.

- An **epoch authority**, which is enforced by the storage device itself. The epoch authority withholds access to data by advancing an "epoch". This is done by zeroizing and later regenerating one of two "epoch keys" held within the device. All epoch keys must be non-zeroized in order for the drive to accept user data. KMB can report the zeroization state of the epoch keys and therefore whether the drive is clean of user data.
  - KMB represents this authority in terms of the **Epoch Authority Key (EAK)**, which itself is a concatenation of the **Soft Epoch Key (SEK)** and **Hard Epoch Key (HEK)**. The SEK and HEK are held in different storage mediums with different rotation and recovery characteristics.
  - The SEK is managed by controller firmware and may be held in flash memory.
  - The HEK is managed by KMB and is held in fuses. This provides assurance that an advanced adversary cannot recover key material that had been in use by the drive prior to the HEK zeroization.

The EAK, DAK, and set of configured MPAKs are used together to derive an MEK secret, which protects a given MEK. The MEK protection is implemented as one of two flows:

- **MEK encryption** - the controller obtains a random MEK from KMB, encrypted by the MEK secret, and can later load the encrypted MEK into the encryption engine via KMB. This supports Opal use-cases.

- **MEK derivation** - the controller instructs KMB to derive an MEK from the MEK secret, then load it into the encryption engine. This may support either Opal or Key Per I/O use-cases.

MEKs are never visible to controller firmware. Controller firmware instructs KMB to load MEKs into the key cache of the encryption engine, using standard interfaces described in the [Interfaces](#interfaces) section. Each MEK has associated vendor-defined metadata, to identify e.g. the namespace and LBA range to be encrypted by the MEK.
...
PMEKs are the mechanism by which KMB enforces multi-party authorization as a precondition to loading an MEK. PMEKs exist in one of two states: locked or ready. In both these states the PMEK is encrypted to a key known only to KMB.
...
- A ready PMEK's encryption key (the Ready PMEK Encryption Key) is a volatile key held within KMB which is lazily generated. Ready PMEKs are held in controller firmware memory. Lazy generation allows injected host entropy to contribute to the key's generation.

The externally-supplied access key is encrypted in transit using an HPKE public key held by KMB. The "ready" state allows the HPKE keypair to be rotated after the access key has been provisioned to the storage device, without removing the ability for KMB to decrypt the PMEK when later loading an MEK bound to that PMEK.

For each PMEK to which a given MEK is bound, the host is expected to invoke a command to supply the PMEK's encrypted access key. Upon receipt the controller firmware passes that encrypted access key to KMB, along with the locked PMEK, to produce the ready PMEK which is cached in controller memory. This is done prior to the controller firmware actually loading the MEK, and is performed once for each PMEK to which a given MEK is bound.
...
<p align="center">
  <img src="./diagrams/pmek_access_key_ecdh_unwrap.drawio.svg" alt="Access key ECDH unwrap" />
</p>
...
<p align="center">
  <img src="./diagrams/pmek_access_key_hybrid_unwrap.drawio.svg" alt="Access key hybrid unwrap" />
</p>
...
1. Unwrap the given PMEK access key using the HPKE keypair held within KMB.
...
Encrypted PMEKs stored at rest in persistent storage are considered "locked", and must be made ready before they can be used to load an MEK. Ready PMEKs are also encrypted when handled by controller firmware, to the Ready PMEK Encryption Key. Ready PMEKs do not survive across device reset.
...
<p align="center">
  <img src="./diagrams/pmek_access_key_rotation.drawio.svg" alt="PMEK access key rotation" />
</p>
...
Controller firmware provides the DEK (Data Encryption Key) to KMB when generating, loading, or deriving an MEK. The DEK is used in a KDF, along with the FEK, CEK, and PMEKs, to [derive](#mek-secret-derivation) the MEK secret.
...
<p align="center">
  <img src="./diagrams/dek_decryption.drawio.svg" alt="DEK decryption" />
</p>

*<p style="text-align: center;">Figure 8: Example controller flow to decrypt a DEK based on a host-provided Opal C_PIN</p>*
...
*<p style="text-align: center;">Figure 9: Example controller flow to accept an injected DEK.</p>*
...
*<p style="text-align: center;">Figure 10: MEK encryption and decryption</p>*
...
*<p style="text-align: center;">Figure 11: MEK derivation</p>*

<div style="page-break-after: always"></div>

### MEK secret derivation

This diagram provides additional details on how the CEK, DEK, FEK, and PMEKs are mixed together to produce the MEK secret, which then either encrypts/decrypts a random MEK or is used to compute a derived MEK.

<p align="center">
  <img src="./diagrams/mek_secret_derivation.drawio.svg" alt="Deriving the MEK secret" />
</p>

*<p style="text-align: center;">Figure 12: MEK secret derivation</p>*
...
The FEK is derived from a series of N 256-bit ratchet secrets present in a fuse bank, dubbed R<sub>0</sub>..R<sub>N-1</sub>, where N is between 4 and 16 inclusive. The vendor is responsible for determining the number of ratchet secrets available in fuses. These ratchet secrets have the following properties:
...
*<p style="text-align: center;">Figure 13: FEK derivation</p>*
...
*<p style="text-align: center;">Figure 14: FEK & CEK state machine with four ratchet slots</p>*
...
KMB generates multiple kinds of random keys. It does so using randomness obtained from a DRBG, which is seeded from Caliptra's TRNG and which may be reseeded with entropy from the host.

<p align="center">
  <img src="./diagrams/drbg_diagram.drawio.svg" alt="Random key generation" />
</p>

*<p style="text-align: center;">Figure 15: DRBG usage</p>*

## Boot-time initialization

At initial boot, KMB firmware performs the following steps:

- Determines whether an FEK is available, based on the fuse configuration. If an FEK is unavailable, KMB enters a mode where MEKs are not allowed to be loaded.
...
*<p style="text-align: center;">Figure 16: KMB to Encryption SFR Interface</p>*
...
*<p style="text-align: center;">Figure 17: LBA range based metadata format</p>*
...
*<p style="text-align: center;">Figure 18: Key tag based metadata format</p>*
...
*<p style="text-align: center;">Figure 19: Auxiliary data format example</p>*
...
*<p style="text-align: center;">Figure 20: MEK format example for AES-XTS-512</p>*
...
*<p style="text-align: center;">Figure 21: Command execution example for loading an MEK</p>*
...
This command reports the state of the epoch keys. The controller indicates the state of the CEK, while KMB internally senses the state of the FEK.
...
Each mailbox command that causes a command to execute on the encryption engine includes a timeout value is specified by the command. KMB aborts the command executing on the encryption engine if the encryption engine does not complete the command within the specified timeout and reports a LOCK_ENGINE_TIMEOUT result code.

Table 6 defines the additional mailbox result codes that may be returned by KMB.
...
<p align="center">
  <img src="./diagrams/uml/power_on.svg" alt="Power on" />
</p>
